      ...
      <div className="relative z-10 pt-24 pb-12 px-4 animate-fade mx-auto max-w-6xl">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-4 space-y-6">
             <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center shadow-xl shadow-black/50">
                <div className="w-24 h-24 rounded-full bg-zinc-800 mx-auto mb-4 overflow-hidden border-2 border-zinc-700 ring-4 ring-black/50">
                   <img
             ...
             <button 
                onClick={logout}
                className="w-full flex items-center gap-3 px-6 py-4 rounded-xl transition-all text-sm font-medium border border-transparent text-red-400 hover:bg-red-900/20 hover:border-red-900/10"
             >
          ...
          <div className="md:col-span-8">
             {activeTab === 'blogs' && (
               <div className="space-y-6">
                 ...
                 {blogs.length === 0 ? (
                    <div className="text-center py-24 border border-dashed border-zinc-800 rounded-2xl bg-zinc-900">
                    ...
                    blogs.map((blog) => (
                      <div key={blog._id} className="group bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 rounded-2xl p-6 transition-all shadow-lg shadow-black/5">
                         ...
                         <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
                            ...
                            <div className="flex gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                               <Link to={`/blog/${blog._id}/edit`} className="p-2 text-zinc-400 hover:text-white hover:bg-zinc-700 rounded-lg transition-colors"><PencilLine weight="regular" size={18}/></Link>
                               <button onClick={() => setDeleteConfirm(blog._id)} className="p-2 text-zinc-400 hover:text-red-400 hover:bg-red-900/20 rounded-lg transition-colors"><Trash weight="regular" size={18}/></button>
                            </div>
                         </div>
                      </div>
                    ))
             ...
             {activeTab === 'analytics' && (
                <div className="grid grid-cols-2 gap-4">
                   ...
                   <div className="p-8 bg-zinc-900 border border-zinc-800 rounded-2xl col-span-2">
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8">
                   ...
                   <div className="space-y-8">
                      <div>
                         ...
                         <input type="text" value={user?.name} disabled className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-zinc-400 cursor-not-allowed font-light" />
                      </div>
                      <div>
                         <label className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-3">Email Address</label>
                         <input type="email" value={user?.email} disabled className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-zinc-400 cursor-not-allowed font-light" />
                      </div>
                   </div>
                </div>
             )}
